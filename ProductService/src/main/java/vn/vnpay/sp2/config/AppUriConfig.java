...
public class AppUriConfig {
    ...
    public static AppUriConfig getInstance() {
        if (Objects.isNull(instance)) {
            try {
                instance = initInstance(PATH_FILE_CONFIG.getProductServiceFileConfigPath());
            } catch (IOException e) {
    ...
    public static AppUriConfig initInstance(String path) throws IOException {
        ...
        try {
            if (StringUtils.isBlank(path)) {
                path = PATH_FILE_CONFIG.getProductServiceFileConfigPath();
            }
